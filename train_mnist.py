...
print(f"Tệp tin JSON đã được tạo tại: {config_path}")
...
for i in range(epochs):
    ...
    for j, data in enumerate(dataloader):
        ...
        # loading batch
        x = data[0].to(device)*255
        
        # pre-processing data
        x = glow.preprocess(x)
        n,c,h,w = x.size()
        final_loss, nll,logdet,logpz,z_mu,z_std = glow.nll_loss(x)
        if global_step == 0:
            global_step += 1
            continue
        ...
        print("\repoch=%0.2d..final_loss=%0.2f..logdet=%0.2f..logpz=%0.2f..mu=%0.2f..std=%0.2f"
