...
class ActNorm(nn.Module):
    ...
    def initialize(self, x):
        if not self.training:
            return
        with torch.no_grad():
            b_    = x.clone().mean(dim=0,keepdim=True).mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)  #mean theo 3 chiều batchsize, height, width 
            s_    = ((x.clone() - b_)**2).mean(dim=0,keepdim=True).mean(dim=2,keepdim=True).mean(dim=3,keepdim=True) # trung bình của phương sai theo 3 chiều 
            b_    = -1 * b_
