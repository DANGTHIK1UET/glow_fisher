...
class Glow(nn.Module):
    ...
    def nll_loss(self, x, logdet=None, retrain=False):
        n,c,h,w = x.size()
        z, logdet, actloss = self.forward(x,logdet=logdet,reverse=False)
        fisher_loss = FisherLoss(x, z, 1 if retrain else 0)
        if not self.init_resizer:
            self.sizes = [t.size() for t in z]
            self.init_resizer = True
        ...
        final_loss = fisher_loss.compute_fisher_loss() + nll
        return final_loss, nll, -logdet.mean().item(),-logpz.mean().item(), z_.mean().item(), z_.std().item()
